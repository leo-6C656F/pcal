...
import type { ModelLoadingState, AIGenerationSettings, AIProviderMode, OpenAISource } from '../types';
...
export function AISettings() {
  ...
  const [providerMode, setProviderMode] = useState<AIProviderMode>('local');
  const [openAISource, setOpenAISource] = useState<OpenAISource>('proxy');
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
        ...
        setAiEnabled(config.aiEnabled !== false); // Default true
        setProviderMode(config.providerMode || 'local');
        setOpenAISource(config.openAISource || 'proxy');
  ...
  const handleProviderModeChange = (mode: AIProviderMode) => {
    ...
    // If switching to local and model not ready, prompt to download
...
Final sentence:`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-lg ${aiEnabled ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>
              <Power size={24} />
            </div>
            <div>
          ...
          <button
            ...
            className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors ${
              aiEnabled ? 'bg-green-500' : 'bg-slate-300'
            }`}
          >
            <span
      ...
      {aiEnabled && (
        <div className="card">
          <div className="mb-4">
            ...
            <p className="text-sm text-slate-500">Choose how AI summaries are generated</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Local Option */}
            <button
              onClick={() => handleProviderModeChange('local')}
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-lg ${providerMode === 'local' ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-600'}`}>
                  <Cpu size={20} />
                </div>
                <span className="font-semibold text-slate-900">Local Model</span>
                {providerMode === 'local' && <CheckCircle size={18} className="ml-auto text-primary" />}
              </div>
              <p className="text-sm text-slate-600">
                Runs on your device. Free, private, works offline. Requires downloading a model (~80-300 MB).
              </p>
            </button>

            {/* OpenAI Option */}
            <button
              onClick={() => handleProviderModeChange('openai')}
              className={`p-4 rounded-lg border-2 text-left transition-all ${
                providerMode === 'openai'
                  ? 'border-primary bg-primary/5'
                  : 'border-slate-200 hover:border-slate-300'
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-lg ${providerMode === 'openai' ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-600'}`}>
                  <Cloud size={20} />
                </div>
                <span className="font-semibold text-slate-900">OpenAI Cloud</span>
                {providerMode === 'openai' && <CheckCircle size={18} className="ml-auto text-primary" />}
              </div>
              <p className="text-sm text-slate-600">
                Uses OpenAI's GPT models. Higher quality, requires internet. Can use our proxy or your own API key.
              </p>
            </button>
          </div>
        </div>
      ...
      {/* Local Model Settings - Only shown when Local is selected */}
      {aiEnabled && providerMode === 'local' && (
        <div className="card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button
              onClick={modelStatus === 'ready' ? handleRedownload : handleDownloadModel}
            ...
            >
              {isRedownloading ? (
                ...
                <>
                  <Download size={18} />
                  <span>{modelStatus === 'ready' ? t('aiSettings.redownload') : 'Download Model'}</span>
                </>
              )}
            </button>
      ...
      {/* OpenAI Settings - Only shown when OpenAI is selected */}
      {aiEnabled && providerMode === 'openai' && (
        <div className="card">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Configure how to connect to OpenAI
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {/* Proxy Option (Default) */}
                <button
                  onClick={() => setOpenAISource('proxy')}
                  className={`p-4 rounded-lg border-2 text-left transition-all ${
                    openAISource === 'proxy'
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <Shield size={18} className={openAISource === 'proxy' ? 'text-blue-600' : 'text-slate-500'} />
                    <span className="font-medium text-slate-900">Secure Proxy</span>
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Recommended</span>
                  </div>
                  <p className="text-xs text-slate-600">
                    Uses our secure server. No API key needed. Works automatically when signed in.
                  </p>
                </button>

                {/* Direct API Option */}
                <button
                  onClick={() => setOpenAISource('direct')}
                  className={`p-4 rounded-lg border-2 text-left transition-all ${
                    openAISource === 'direct'
                      ? 'border-blue-500 bg-blue-50'
                      : 'border-slate-200 hover:border-slate-300'
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <Key size={18} className={openAISource === 'direct' ? 'text-blue-600' : 'text-slate-500'} />
                    <span className="font-medium text-slate-900">Your Own API Key</span>
                  </div>
                  <p className="text-xs text-slate-600">
                    Use your own OpenAI API key. Useful for custom limits, Azure, or other providers.
                  </p>
                </button>
              </div>
            </div>

            {/* Direct API Settings - Only shown when direct is selected */}
            {openAISource === 'direct' && (
              <div className="p-4 bg-slate-50 rounded-lg space-y-4">
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    Custom endpoint for Azure OpenAI, LocalAI, or other compatible services.
                  </p>
                </div>
              </div>
            )}
            ...
            {/* OpenAI-specific Advanced Settings */}
            <div className="pt-4 border-t border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                <Settings2 size={18} className="text-slate-600" />
                <h4 className="text-sm font-semibold text-slate-700">Advanced OpenAI Settings</h4>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-primary">{genSettings.frequencyPenalty.toFixed(1)}</span>
                  </div>
                  <input
                    type="range"
                    min="-2.0"
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    Reduces repetition based on token frequency. Positive values decrease likelihood of repeating words.
                  </p>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-primary">{genSettings.presencePenalty.toFixed(1)}</span>
                  </div>
                  <input
                    type="range"
                    min="-2.0"
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    Encourages new topics. Positive values increase likelihood of discussing new subjects.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    value={genSettings.systemMessage}
                    onChange={(e) => handleSettingChange('systemMessage', e.target.value)}
                    placeholder="E.g., 'You are a helpful assistant that writes concise summaries.'"
                    rows={3}
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    Sets the behavior of the AI. Leave empty to use the default system behavior.
                  </p>
                </div>
              </div>
            </div>
            ...
            {openAISource === 'proxy' && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-900">
                  <strong>Secure Mode:</strong> Using our authenticated proxy means your requests are processed through our server with our API key. No setup required - just sign in and use AI features.
                </p>
              </div>
            )}
      ...
      {/* Prompt Editor - Only shown when AI is enabled */}
      {aiEnabled && (
        <div className="card">
      ...
      {/* Generation Settings Card - Only shown when AI is enabled */}
      {aiEnabled && (
        <div className="card">
          ...
          <div className="space-y-6">
            ...
            {/* Min Length - Only for local models */}
            {providerMode === 'local' && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium text-slate-700">
